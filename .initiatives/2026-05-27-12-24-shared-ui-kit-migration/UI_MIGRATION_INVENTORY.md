...
This inventory is now both a migration record and a legacy allowlist reference. The broad shared primitives exist in `src/client/components/ui/UiComponents.ts`, while the remaining helper files preserve compatibility for existing call sites.

The static guardrail is `tests/client/components/UiMigrationGuard.test.ts`. It scans `src/client/components` for new raw controls and obvious ad hoc surface styling. Existing legacy files are allowlisted there; adding a new file to that allowlist should be treated as a deliberate design exception, not a default path.

Known intentional exceptions:

- `src/client/components/baseComponents/Button.ts`: `<o-button>` remains for translation-key compatibility and now composes `ui-button` / `ui-icon-button`.
- `src/client/components/baseComponents/Modal.ts`: `<o-modal>` remains for existing modal lifecycle, tab, inline, and body-scroll behavior.
- `src/client/components/ui/ActionButton.ts`: helper remains for existing player-panel action grids and composes `ui-button`.
- `src/client/components/ui/ModalHeader.ts`: helper remains for older modal bodies that render their own layout.
- `src/client/components/ui/Divider.ts`: light-DOM separator remains for Tailwind-dependent legacy panel markup.
- HUD overlays can use app `ui-*` primitives for generic dialog/feed controls, while gameplay-dense control panels, meters, attack rows, event rows, and unit buttons stay on `hud-*`.
...
- Current kit usage: composes `ui-button` while preserving the existing player action helper API.
- Convert to: keep as a compatibility helper until player-panel action grids move to direct primitive composition.
...
- Current kit usage: widely used by root app modals and composes `ui-icon-button`.
- Convert to: prefer `ui-modal-header` for new modal shells; keep this helper for older bodies that provide their own section layout.
...
- Current kit usage: compatibility wrapper around `ui-button` / `ui-icon-button`.
- Convert to: use `ui-button` directly for new work unless translation-key compatibility is needed.
...
- Current kit usage: compatibility shell that preserves legacy open/close, tab, inline, and body-scroll behavior while composing shared icon-button behavior.
- Convert to: use `ui-modal-shell` for new modal surfaces; migrate remaining `<o-modal>` call sites only when lifecycle behavior is covered.
